...
def predict(text):
    ...
    if text!= "":
        ...
        padded = pad_sequences(sequences, maxlen=max_length, padding=padding_type, tuncating = trunc_type)
        testing_padded = np.array(padded)
        
        predicted_class_label = np.argrax(model.predict(testing_padded),axxis = 1)
